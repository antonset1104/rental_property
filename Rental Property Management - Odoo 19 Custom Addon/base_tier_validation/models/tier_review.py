...
class TierReview(models.Model):
    ...
    def read(self, fields=None, load='_classic_read'):
        """Add logging to debug field reading"""
        result = super().read(fields, load)
        for record_data in result:
            if 'requested_by' in record_data or 'done_by' in record_data:
                _logger.info(f"TierReview.read() - ID: {record_data.get('id')}")
                _logger.info(f"  requested_by: {record_data.get('requested_by')}")
                _logger.info(f"  done_by: {record_data.get('done_by')}")
                _logger.info(f"  name: {record_data.get('name')}")
                _logger.info(f"  status: {record_data.get('status')}")
        return result

    @api.model_create_multi
    def create(self, vals_list):
        """Add logging to debug field creation"""
        for vals in vals_list:
            _logger.info(f"TierReview.create() - Values: {vals}")
        result = super().create(vals_list)
        for record in result:
            _logger.info(f"TierReview.create() - Created ID: {record.id}")
            _logger.info(f"  requested_by: {record.requested_by.id if record.requested_by else False}")
            _logger.info(f"  done_by: {record.done_by.id if record.done_by else False}")
        return result

    def write(self, vals):
        """Add logging to debug field updates"""
        if 'requested_by' in vals or 'done_by' in vals:
            _logger.info(f"TierReview.write() - IDs: {self.ids}")
            _logger.info(f"  Values: {vals}")
        result = super().write(vals)
        if 'requested_by' in vals or 'done_by' in vals:
            for record in self:
                _logger.info(f"TierReview.write() - Updated ID: {record.id}")
                _logger.info(f"  requested_by: {record.requested_by.id if record.requested_by else False}")
                _logger.info(f"  done_by: {record.done_by.id if record.done_by else False}")
        return result
