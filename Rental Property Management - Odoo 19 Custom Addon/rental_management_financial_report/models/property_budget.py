...
class PropertyBudget(models.Model):
    ...
    # Allow base_tier_validation to auto-inject Validate/Reject buttons,
    # the status alert banner, and the review_ids widget into the form view.
    ...
    # H1: optional owner filter — budget figures are prorated by ownership % when filtered
    owner_id = fields.Many2one('res.partner', string='Owner (Filter)',
                               help="Leave empty for a property-level budget. "
                                    "If set, variance reports can be filtered to show "
                                    "this owner's share (ownership % × budget amount).")
    ...
    def _validate_tier(self, tiers=None):
        """Override: setelah semua tier approved, otomatis set state ke approved."""
