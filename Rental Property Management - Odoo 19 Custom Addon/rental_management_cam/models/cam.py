...
class PropertyCamPeriod(models.Model):
    ...
    def action_create_true_up_reconciliation(self):
        """PDF 2 item 48-49: Year-End True-Up Reconciliation.
        Compares actual expense share vs previously invoiced budget amounts.
        Creates balancing invoices (under-billed) or credit notes (over-billed)."""
        for cam in self:
            product = cam.product_id or self.env.ref(
                'rental_management_cam.product_cam', raise_if_not_found=False)
            if not product:
            ...
            count_inv = count_refund = 0
            for line in cam.tenant_line_ids:
                variance = line.true_up_variance or 0.0
                if abs(variance) < 0.01 or line.balancing_move_id:
                    continue
                if variance > 0:
                    # Tenant under-billed -> Issue Balancing Customer Invoice
                    move_vals = {
                        'move_type': 'out_invoice',
                        'partner_id': line.partner_id.id,
                        'invoice_date': cam.date_to or fields.Date.today(),
                        'invoice_line_ids': [(0, 0, {
                            'product_id': product.id,
                            'name': self.env._('Year-End CAM True-Up (Under-Billed Adjustment) - %s (%s)') % (
                                cam.name, cam.property_id.name or ''),
                            'quantity': 1.0,
                            'price_unit': round(variance, 2),
                        })],
                    }
                    count_inv += 1
                else:
                    # Tenant over-billed -> Issue Credit Note / Refund
                    move_vals = {
                        ...
                        'invoice_line_ids': [(0, 0, {
                            'product_id': product.id,
                            'name': self.env._('Year-End CAM True-Up (Over-Billed Credit) - %s (%s)') % (
                                cam.name, cam.property_id.name or ''),
                            'quantity': 1.0,
                            'price_unit': round(abs(variance), 2),
                        })],
                    }
                    count_refund += 1
                ...
                line.balancing_move_id = move.id

            cam.message_post(body=self.env._(
                "⚖️ <b>Rekonsiliasi Akhir Tahun (CAM True-Up) Selesai</b>.<br/>"
                "Diterbitkan %s Balancing Invoice (kurang bayar) dan %s Credit Note (lebih bayar).") % (
                    count_inv, count_refund))
        return True
...
class PropertyCamTenantLine(models.Model):
    ...
    amount = fields.Monetary(string='Realisasi Aktual Share', compute='_compute_amount', store=True)
    invoiced_budget_amount = fields.Monetary(string='Sudah Ditagih (Budget)')
    true_up_variance = fields.Monetary(string='Selisih True-Up', compute='_compute_true_up_variance', store=True)
    invoice_id = fields.Many2one('account.move', string='Invoice Awal', readonly=True)
    balancing_move_id = fields.Many2one('account.move', string='Balancing Invoice/Credit Note', readonly=True)
    currency_id = fields.Many2one(related='cam_id.currency_id')
    company_id = fields.Many2one(related='cam_id.company_id', store=True)
    ...
    @api.depends('amount', 'invoiced_budget_amount')
    def _compute_true_up_variance(self):
        for l in self:
            l.true_up_variance = (l.amount or 0.0) - (l.invoiced_budget_amount or 0.0)
